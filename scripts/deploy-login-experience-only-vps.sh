...
STAGE="/var/www/kalpavriksha-login-experience-${STAMP}"
BACKUP="/var/backups/kalpavriksha-runtime/${STAMP}-login-experience"
...
rollback(){
  ...
  trap - ERR INT TERM
  echo
  echo "LOGIN EXPERIENCE DEPLOYMENT FAILED WITH CODE $rc"
  if [[ "$SWITCHED" == "1" ]]; then
    log "Restoring the previous login backend and frontend"
...
log "Checking the existing live runtime"
test -d "$LIVE/.git"
...
log "Extracting the target without changing the live checkout"
rm -rf "$STAGE"
mkdir -p "$STAGE"
git -C "$LIVE" archive "$TARGET_COMMIT" | tar -x -C "$STAGE"
ln -s "$LIVE/backend/node_modules" "$STAGE/backend/node_modules"

log "Running only login, role, session and workspace preflight checks"
node --check "$STAGE/backend/src/server.js"
node --check "$STAGE/frontend/src/services/authService.js"
bash -n "$STAGE/scripts/deploy-login-experience-only-vps.sh"
(
  cd "$STAGE"
  node --test \
    tests/frontend/login-experience-hardening.test.mjs \
    tests/frontend/session-performance.test.mjs \
    tests/frontend/profile-workspace-role-safety.test.mjs \
    tests/backend/login-experience-hardening.test.mjs \
    tests/backend/session-performance-hardening.test.mjs \
    tests/backend/profile-registration-and-role-journey.test.mjs \
    tests/backend/login-experience-deployment.test.mjs
...
log "Stopping the backend for the short atomic login-runtime switch"
...
cat > /var/lib/kalpavriksha/login-experience-release.json <<JSON
{
  "commit":"$TARGET_COMMIT",
  "deployedAt":"$(date -u +%Y-%m-%dT%H:%M:%SZ)",
  "deploymentType":"latest-changes-only-login-experience",
...
log "LOGIN EXPERIENCE LATEST-CHANGES DEPLOYMENT COMPLETED"
