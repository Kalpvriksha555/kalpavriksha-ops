...
import { ONLINE_STALE_MS } from '../config/presenceConfig.js';
...
export const getOperationalUsers = (users = [], { includeAdmins = true } = {}) => asArray(users)
...
export const normalizeChatReadBy = (value) => {
  if (Array.isArray(value)) return value.filter(entry => entry !== null && entry !== undefined && entry !== '');
  if (value === null || value === undefined || value === '') return [];
  return [value];
};
export const hasReadBy = (message, userName) => normalizeChatReadBy(message?.readBy).some(r => samePerson(readEntryName(r), userName));
