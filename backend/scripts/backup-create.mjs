...
let lockFd;
try{lockFd=fs.openSync(lockPath,'wx');}catch{throw new Error(`Another backup appears to be running (${lockPath}).`);}
...
try{
  if(dryRun){console.log(JSON.stringify({ok:true,dryRun:true,id,backupRoot,database:!filesOnly,files:!databaseOnly},null,2));process.exitCode=0;}
    ...
    if(!filesOnly){
      ...
      run(process.env.PG_DUMP_BIN||'pg_dump',['--format=custom','--compress=6','--no-owner','--no-acl','--file',partial,databaseUrl]);
    ...
    if(!databaseOnly){
      ...
      run(process.env.TAR_BIN||'tar',['--exclude=./temp','-czf',partial,'-C',storageRoot,'.']);
