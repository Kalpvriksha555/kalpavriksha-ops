...
const seed = {
  users:[
    ...
    {id:'u-manager',name:'Operations Manager',role:'MANAGER',phone:'919999999990'},
...
function norm(d){
  ...
  d.users ||= seed.users; d.cases ||= d.projects || []; d.payments ||= []; d.notifications ||= []; d.teamChat ||= d.chatMessages || []; d.whatsappInbox ||= []; d.audit ||= []; d.attendanceLogs ||= []; d.chatReads ||= {ADMIN:[],MANAGER:[],DESIGNER:[]};
  if(!d.users.find(u=>u.role==='MANAGER')) d.users.splice(1,0,seed.users[1]);
...
function leastBusy(d){ return d.users.filter(u=>u.role==='DESIGNER').map(u=>({ ...u, active:d.cases.filter(c=>c.assigneeId===u.id && !['COMPLETED','CLOSED'].includes(c.status)).length })).sort((a,b)=>a.active-b.active)[0] || d.users.find(u=>u.role==='MANAGER'); }
