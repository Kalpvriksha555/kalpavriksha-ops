...
const seed = {
  users:[
    {id:'u-admin',name:'Admin',role:'ADMIN',phone:'917017813662'},
    {id:'d-ashu',name:'Ashutosh Rai',role:'DESIGNER',phone:'919999999991'},
    {id:'d-ravi',name:'Ravi Kumar',role:'DESIGNER',phone:'919999999992'}
  ],
  cases:[], payments:[], notifications:[], teamChat:[], whatsappInbox:[], audit:[], attendanceLogs:[], chatReads:{ADMIN:[],MANAGER:[],DESIGNER:[]}
};
...
function isSystemPlaceholderUser(u = {}) {
  const name = String(u.name || '').trim().toLowerCase();
  const username = String(u.username || '').trim().toLowerCase();
  const id = String(u.id || '').trim().toLowerCase();
  return name === 'operations manager' || username === 'operationsmanager' || id === 'u-manager';
}
function visibleUsers(users = []) {
  return (users || []).filter(u => !isSystemPlaceholderUser(u));
}

function norm(d){
  d ||= structuredClone(seed);
  d.users ||= seed.users; d.users = visibleUsers(d.users); d.cases ||= d.projects || []; d.payments ||= []; d.notifications ||= []; d.teamChat ||= d.chatMessages || []; d.whatsappInbox ||= []; d.audit ||= []; d.attendanceLogs ||= []; d.chatReads ||= {ADMIN:[],MANAGER:[],DESIGNER:[]};
...
const presenceTimestamp = (u = {}) => Math.max(
  toMillis(u.lastHeartbeatAt),
  toMillis(u.lastSeenAt),
  toMillis(u.lastLoginAt),
  toMillis(u.availabilityUpdatedAt),
  toMillis(u.lastLogoutAt)
);
...
function sanitizePresenceUsers(users = []) {
  ...
  return (users || []).map(u => sanitizePresenceUser(u, nowMs));
}
function mergeUsersPreservingLatestPresence(existing = [], incoming = []) {
  const byId = new Map();
  const add = (u = {}) => {
    const key = String(u.id || u.username || u.name || Math.random());
...
function leastBusy(d){ return visibleUsers(d.users).filter(u=>u.role==='DESIGNER').map(u=>({ ...u, active:d.cases.filter(c=>c.assigneeId===u.id && !['COMPLETED','CLOSED'].includes(c.status)).length })).sort((a,b)=>a.active-b.active)[0] || visibleUsers(d.users).find(u=>u.role==='MANAGER'); }
...
const app=express();
app.use(cors({origin:true})); app.use((req,res,next)=>{ res.setHeader('Cache-Control','no-store, no-cache, must-revalidate, private'); next(); }); app.use(express.json({limit:'30mb'})); app.use('/uploads',express.static(UPLOAD_DIR));
...
app.get('/api/state',(req,res)=>{
  ...
  res.json({
    ...
    users:sanitizePresenceUsers(visibleUsers(d.users || [])),
    projects:d.cases || [],
...
app.delete('/api/state/projects/:id', (req,res)=>{
  ...
  d.cases=(d.cases || []).filter(c => String(c.id) !== id && String(c.caseId) !== id);
  save(d);
  res.json({ok:true, deleted: before - d.cases.length, counts:{cases:d.cases.length}});
});

app.post('/api/state',(req,res)=>{
  const d=db();
  const body=req.body || {};
  d.users = visibleUsers(Array.isArray(body.users) ? mergeUsersPreservingLatestPresence(d.users || [], body.users) : sanitizePresenceUsers(d.users || []));
  d.cases = Array.isArray(body.projects) ? body.projects : (Array.isArray(body.cases) ? body.cases : d.cases);
  ...
  save(d);
  res.json({ok:true, database: USE_POSTGRES ? 'postgresql' : 'json-file', savedAt:now(), counts:{users:d.users.length, cases:d.cases.length, chatMessages:d.teamChat.length, notifications:d.notifications.length, attendanceLogs:d.attendanceLogs.length}});
});
...
app.post('/api/cases', upload.array('files',20), async (req,res)=>{
  ...
  const manager=visibleUsers(d.users).find(u=>u.role==='MANAGER');
