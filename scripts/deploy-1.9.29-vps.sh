...
WORK="${KALPA_DEPLOY_WORK:-/root/kalpavriksha-1.9.29-deploy-$(date -u +%Y%m%dT%H%M%SZ)}"
...
EXPECTED_ROOT_VERSION="1.9.29-final-health-probe-closure"
EXPECTED_BACKEND_VERSION="2.9.29-final-health-probe-closure"
EXPECTED_FRONTEND_VERSION="2.9.29-final-health-probe-closure"
...
git -C "$RELEASE_ROOT" -c user.name='Kalpavriksha Release' -c user.email='release@localhost' \
  commit -qm "Kalpavriksha 1.9.29 verified ZIP release"
...
bash -n scripts/deploy-1.9.29-vps.sh
...
echo "======================================================"
echo "KALPAVRIKSHA 1.9.29 DEPLOYMENT COMPLETED SUCCESSFULLY"
echo "======================================================"
